...
auto move3 = controller.createMove(std::nullopt, to, std::nullopt, piece, std::nullopt);
```
...
### Q. `std::optional<Piece>` は `ViewPiece` として DTO 定義すべき？それとも Square として扱うべき？

A. **Square（マス）として扱うべき** です。これは概念の混乱を避けるための重要な整理です。

#### 現在の問題点

```cpp
// ❌ 概念が混乱している
std::optional<Piece> piece = board.getPiece(pos);  // これは「駒」ではなく「マス」では？

struct ViewPiece {  // 「駒」なのに「マスの状態」を持っている
    bool hasPiece;  // 駒があるかどうか = これはマスの情報
    PieceType pieceType;
    PlayerSide owner;
};
```

**問題**: `std::optional<Piece>` も `ViewPiece` も、実際には「駒があるかもしれないマス」を表現しているのに、名前が「Piece（駒）」になっている
...
private:
    std::array<std::array<Square, BoardSize>, BoardSize> layout_;
};
```

**View 層 (DTO)**:
```cpp
// ViewSquare = View 用のマス
struct SHORYU_API ViewSquare {
    bool hasPiece = false;
    PlayerSide owner = PlayerSide::Sente;
...
| 概念 | Model 層 | View 層 | 説明 |
|------|---------|---------|------|
| **駒** | `Piece` | `PieceType` + `PlayerSide` | 駒そのもの |
| **マス** | `Square` = `std::optional<Piece>` | `ViewSquare` | 駒があるかもしれない場所 |
| **盤面** | `Board` | `ViewBoardLayout` | マスの集合 (9×9) |

#### メリット

| 観点 | 改善点 |
|------|--------|
| **概念の明確化** | Square = マス、Piece = 駒と明確に分離 |
| **コードの可読性** | `getSquare(pos)` は直感的で分かりやすい |
| **型の意図** | `using Square = std::optional<Piece>` で意図が明確 |
| **一貫性** | Model 層と View 層で概念が統一される |
| **nullopt チェックの意味** | 「駒がない」ではなく「マスが空」と理解できる |


#### まとめ

- `std::optional<Piece>` は「マス（Square）」を表現している
- `ViewPiece` も「マス（ViewSquare）」として再定義すべき
- 「Piece（駒）」と「Square（マス）」を明確に区別することで、コードの意図が明確になる
- この概念整理は将来的に実装する価値がある
...
## C++/CLI Interop 層で enum を定義し直す理由

### Q. ネイティブの enum（`PieceType`, `PlayerSide`）を C++/CLI でマネージド enum として再定義すべき？

A. **Yes。マネージド enum として再定義すべき** です（強く推奨）。

### 選択肢の比較

| 選択肢 | メリット | デメリット | 推奨度 |
|-------|---------|-----------|--------|
| **マネージド enum 定義** | 型安全・自然な C# コード・独立性 | 定義の重複・変換コード必要 | ✅ **推奨** |
| ネイティブ enum 公開 | 定義の重複なし・変換不要 | 名前空間が不自然・依存増加 | ⚠️ 場合による |
| int で渡す | 実装がシンプル | 型安全性なし・IntelliSense なし | ❌ 非推奨 |

### 1. マネージド enum を定義する（推奨） ✅

**C# から見た使用例**:
```csharp
// 自然で型安全
ManagedPieceType piece = ManagedPieceType.Fu;
ManagedPlayerSide player = ManagedPlayerSide.Sente;
```

**メリット**:
- ✅ **型安全性**: コンパイル時に型チェックが効く
- ✅ **IntelliSense**: IDE の補完が効く（`ManagedPieceType.` と打つと候補が出る）
- ✅ **自然な名前空間**: C# から見ると `Shoryu.Interop.ManagedPieceType` という自然な名前
- ✅ **Core の隠蔽**: `shoryu::core` の内部実装を隠蔽できる
- ✅ **独立性**: 将来 Core の enum 定義を変更しても、Interop 層で吸収できる

**デメリット**:
- ❌ 定義の重複（ネイティブとマネージドで2回書く必要がある）
- ❌ 変換コードが必要

### 2. ネイティブ enum をそのまま公開 ⚠️

**C# から見た使用例**:
```csharp
// 不自然な名前空間
shoryu.core.PieceType piece = shoryu.core.PieceType.Fu;
```
...
**デメリット**:
- ❌ **名前空間が不自然**: C# から `shoryu.core` という内部実装の名前空間が見える
- ❌ **依存増加**: C++/CLI が `core/Types.h` に直接依存する
- ❌ **変更の影響**: Core の変更が直接 C# に影響する
- ❌ **隠蔽の失敗**: Interop 層の役割（Core の隠蔽）を果たせない
...
```csharp
// 何の駒か分からない
int piece = controller.GetPieceType();
if (piece == 0) { /* Fu? */ }
```

**デメリット**:
- ❌ **型安全性なし**: コンパイラのチェックが効かない
- ❌ **IntelliSense なし**: IDE の補完が効かない
- ❌ **定数定義が必要**: C# 側で `const int Fu = 0;` などを定義する必要がある
- ❌ **可読性が低い**: コードの意図が不明確

### 推奨設計パターン

**マネージド enum 定義 + 変換ヘルパー関数**:
- C++/CLI でマネージド enum を定義
- ネイティブ ⇄ マネージド変換ヘルパー関数（`ToManaged()` / `ToNative()`）を用意
- 変換は Interop プロジェクト内で完結

### C++/CLI で書いた型の本質

**重要**: C++/CLI で書いたマネージド型は、C# で書いた型と**本質的に同じ**です。

```
C++/CLI コード → [C++/CLI コンパイラ] → MSIL (共通中間言語)
                                          ↑ 同じバイナリ
C# コード     → [C# コンパイラ]       → MSIL (共通中間言語)
```

**実行時の動作**:
- C++/CLI で定義した `ManagedPieceType` も
- C# で定義した `enum PieceType` も
- **バイナリレベルで互換**（同じ MSIL を生成）
- .NET ランタイム (CLR) から見ると全く区別がつかない

**C++/CLI の特殊な点**:
唯一の違いは、**ネイティブ C++ と混在できる**こと。C++/CLI では以下が可能:
- ネイティブ型をメンバに持てる
- ネイティブ型をマネージド型に変換できる
- マネージド型を返すメソッドを定義できる

これは C# では**絶対に書けません**（C# はネイティブ型を直接扱えない）。

### Interop プロジェクトの役割

**Interop = C# 向けの .NET 型定義 + Core.dll との橋渡し**

```
C# GUI プロジェクト
  ↓ (参照)
Interop.dll (C++/CLI)
  ├─ マネージド型定義 (C# 互換の .NET 型)
  │   ├─ ManagedPosition
  │   ├─ ManagedPieceType ← enum も含む
  │   ├─ ManagedPlayerSide
  │   ├─ ManagedSquare
  │   └─ GameControllerManaged
  └─ ネイティブ⇄マネージド変換ロジック
       ↓
Core.dll (ネイティブ C++)
```
...
**Interop プロジェクトで行うこと**:
1. **C# 向けの .NET 型定義**（マネージド型・enum を含む）
2. **ネイティブ ⇄ マネージド変換**（薄いラッパー）

**Interop プロジェクトでやらないこと**:
- ビジネスロジック（将棋のルール実装）→ Core.dll の責務
- データ構造の実装（Board, Hand など）→ Core.dll の責務
- UI 処理 → C# GUI の責務

### まとめ

- **マネージド enum を定義することで、型安全で自然な API を C# に提供できる**
- 定義の重複はあるが、**独立性・保守性・C# 側の使いやすさ**というメリットが大きい
- C++/CLI で書いたマネージド型は **C# で書いた型と本質的に同じ**（MSIL レベルで互換）
- Interop 層の役割は「**Core.dll との通信で使用される C# の型定義を書く**」こと
- enum も struct と同様に、**マネージド版を定義し直すのがベストプラクティス**